...
def get_repulsion_loss4(pred, nsample=20, radius=0.07):
    ...
    tf.compat.v1.summary.histogram('smooth/unque_index', pts_cnt)
    ...
    h = 0.03
    dist_square = tf.reduce_sum(input_tensor=grouped_pred ** 2, axis=-1)
    ...
    uniform_loss = tf.reduce_mean(input_tensor=radius-dist*weight)
    return uniform_loss
...
def get_emd_loss(pred, gt, radius):
    ...
    emd_loss = tf.reduce_mean(input_tensor=dist_norm)
...
def get_cd_loss(pred, gt, radius):
    ...
    CD_dist = tf.reduce_mean(input_tensor=CD_dist, axis=1)
    CD_dist_norm = CD_dist/radius
    cd_loss = tf.reduce_mean(input_tensor=CD_dist_norm)
    return cd_loss,None
