...
def max_pool2d(inputs,
  ...
  with tf.compat.v1.variable_scope(scope) as sc:
    kernel_h, kernel_w = kernel_size
    stride_h, stride_w = stride
    outputs = tf.reduce_max(input_tensor=inputs, axis=1, keepdims=True)
...
def instance_norm(net, train=True,weight_decay=0.00001):
    ...
    shift = tf.compat.v1.get_variable('shift',shape=var_shape,
                            initializer=tf.compat.v1.zeros_initializer,
                            regularizer=tf.keras.regularizers.l2(0.5 * (weight_decay)))
    scale = tf.compat.v1.get_variable('scale', shape=var_shape,
...
def conv2d_ibn(inputs,
  ...
  Returns:
    Variable tensor
  """
  with tf.compat.v1.variable_scope(scope,reuse=reuse) as sc:
      if use_xavier:
          initializer = tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform")
      else:
          initializer = tf.compat.v1.truncated_normal_initializer(stddev=stddev)

      outputs = tf.compat.v1.layers.conv2d(inputs,num_output_channels,kernel_size,stride,padding,
                                 kernel_initializer=initializer,
                                 kernel_regularizer=tf.keras.regularizers.l2(0.5 * (weight_decay)),
                                 bias_regularizer=tf.keras.regularizers.l2(0.5 * (weight_decay)),
                                 use_bias=use_bias,reuse=None)
      assert not (bn and ibn)
      if bn:
          outputs = tf.compat.v1.layers.batch_normalization(outputs,momentum=bn_decay,training=is_training,renorm=False,fused=True)
